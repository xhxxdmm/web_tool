                ...
                <el-row :gutter="24" justify="space-around" class="search-row">
                  <el-col :span="6">
                    <el-form-item label="RecordId">
                      <el-input v-model="userForm.record_id" clearable placeholder="Please input" clearable style="width: 200px" @keyup.enter.native="onSubmit"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6">
                    <el-form-item label="Radio_productName">
                      <el-input v-model="userForm.name" clearable placeholder="Please input" clearable style="width: 200px" @keyup.enter.native="onSubmit"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6">
...
<script>
    import { getUsermu41, addUsermu41, editUsermu41, delUsermu41 } from "../api";
    export default {
        data() {
            return {
                testdata: "",
        ...
        methods: {
            ...
            submit() {
                ...
                this.$refs.form.validate((valid) => {
                    ...
                    if (valid) {
                        // 后续对表单数据的处理
                        if (this.modalType === 0) {
                            addUsermu41(this.form).then(() => {
                                // 重新获取列表的接口
                                this.getList();
                            });
                        } else {
                            editUsermu41(this.form).then(() => {
                                // 重新获取列表的接口
                                this.getList();
                            });
                        }
            ...
            getList() {
                // 获取的列表的数据
                getUsermu41({ params: { ...this.userForm, ...this.pageData, ...this.total } }).then(
                    ({ data }) => {
                        console.log("data");
