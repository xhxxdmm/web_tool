...
<script>
    import { getUsermu48, addUsermu48, editUsermu48, delUsermu48 } from "../api";
    export default {
        data() {
            return {
                testdata: "",
        ...
        methods: {
            ...
            submit() {
                ...
                this.$refs.form.validate((valid) => {
                    ...
                    if (valid) {
                        // 后续对表单数据的处理
                        if (this.modalType === 0) {
                            addUsermu48(this.form).then(() => {
                                // 重新获取列表的接口
                                this.getList();
                            });
                        } else {
                            editUsermu48(this.form).then(() => {
                                // 重新获取列表的接口
                                this.getList();
                            });
                        }
            ...
            getList() {
                // 获取的列表的数据
                getUsermu48({ params: { ...this.userForm, ...this.pageData, ...this.total } }).then(
                    ({ data }) => {
                        console.log("data");
