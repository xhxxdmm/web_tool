...
@CrossOrigin
@RestController
@RequestMapping("radiodata_foureight")
public class FourEightMemberController {

	@Autowired
	private FourEightMemberRepository repository;
	
	@ResponseBody
	@GetMapping("get/{id}")
	public FourEightMember get(@PathVariable Long id) {
		return repository.findOne(id);
//		return repository.findById(id).orElse(null);
	}

	@PostMapping
	public FourEightMember addUser(@RequestBody FourEightMember user) {
		// 处理用户添加逻辑
		FourEightMember savedUser = repository.save(user);
		return savedUser;
	}

	@PostMapping("save")
	public ExecuteDTO save(@RequestBody FourEightMember entity) {
		// if (entity.date == null) {
	...
	public PageResultDTO loadPage(PageQueryParamDTO params) {
		int a=1;
		// 动态查询条件
		Specification<FourEightMember> spec = (root, query, cb) -> {
			// 初始化一个集合来存储所有查询条件
		...
		Page<FourEightMember> pageResult = repository.findAll(spec, pageable);
